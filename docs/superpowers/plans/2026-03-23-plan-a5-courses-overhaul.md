...
npm run dev
```
Use the Chrome DevTools MCP (`chrome-devtools`) to open the app at `http://localhost:8080`. Google login cookies persist in Chromium, so you should already be authenticated — no need to log in again. Navigate to the Courses page (`/courses`).
