  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-[500px] max-h-[90vh] overflow-y-auto p-0 gap-0">
        {/* Accent stripe */}
        <div className="h-[3px] bg-[var(--accent-hex)]" />
        ...
        <div className="px-8 pt-7 pb-6">
          ...
          {/* Success circle */}
          <div className="w-[52px] h-[52px] rounded-full bg-[var(--accent-hex)] flex items-center justify-center mb-4">
          ...
          {/* Title */}
          <DialogTitle className="font-display text-[22px] font-bold text-[var(--ink)] mb-1.5">
          ...
          <div className="mb-1">
            ...
            <div className="flex border-[1.5px] border-hsl(var(--border)) rounded-lg overflow-hidden">
              <input
                value={publishUrl}
                readOnly
                className="flex-1 border-none outline-none text-xs font-mono px-3 py-2 text-[var(--ink)] bg-[var(--canvas)] min-w-0"
              />
              <button
                onClick={() => onCopy(publishUrl, "URL copied!")}
                className="bg-[var(--accent-hex)] border-none text-white text-xs font-bold px-4 cursor-pointer whitespace-nowrap"
              >
                Copy
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 px-8 pb-6">
          {[
            {
              icon: <Package className="w-5 h-5 text-[var(--accent-hex)]" />,
              iconBg: "bg-green-50",
              label: "SCORM 1.2",
            ...
            {
              icon: <Globe className="w-5 h-5 text-[var(--accent-hex)]" />,
              iconBg: "bg-blue-50",
              label: "HTML Export",
            ...
            {
              icon: <FileJson className="w-5 h-5 text-[var(--accent-hex)]" />,
              iconBg: "bg-orange-50",
              label: "Course JSON",
            ...
            >
              ...
              <div className="text-xs font-bold text-[var(--ink)]">{card.label}</div>
