...
export function HotspotForm({ block, onChange }: Props) {
  ...
  const placeHotspotAt = (x: number, y: number) => {
    ...
    <div className="space-y-4">
      ...
      {block.src && (
        <>
          ...
          <p className="text-xs text-muted-foreground">Click the image or press Enter/Space to place a hotspot</p>
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") {
                e.preventDefault();
                // Place hotspot at center of image (50%, 50%)
                placeHotspotAt(50, 50);
              }
            }}
            role="button"
            tabIndex={0}
            aria-label="Click or press Enter/Space to place a hotspot on the image"
          >
            ...
            {block.hotspots.map((h, i) => (
              <div key={h.id} style={{ position: "absolute", left: `${h.x}%`, top: `${h.y}%`, transform: "translate(-50%, -50%)" }}
                className="w-6 h-6 rounded-full bg-[var(--accent-hex)] border-2 border-white text-white text-xs font-bold flex items-center justify-center shadow-md pointer-events-none">
                {i + 1}
              </div>
            ))}
          </div>
              ...
              {block.hotspots.map((h, i) => (
                <div key={h.id} className="rounded-md border border-border p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[var(--accent-hex)] text-white text-xs flex items-center justify-center font-bold shrink-0">{i + 1}</span>
